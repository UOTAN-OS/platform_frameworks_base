...
public class IcuDateTextView extends DoubleShadowTextView {
    ...
    protected void onAttachedToWindow() {
        ...
        if (mUpdatesOnAod) {
            try {
                if (mBgHandler == null) {
                    Log.wtf("IcuDateTextView",
                            "Must set background handler when mUpdatesOnAod is set to avoid making"
                                    + " binder calls on main thread");
                    ContentResolver resolver = getContext().getContentResolver();
                    resolver.registerContentObserver(Settings.Secure.getUriFor("doze_always_on"),
                            false, mAodSettingsObserver, -1);
                } else {
                    mBgHandler.post(() -> {
                        ContentResolver resolver = getContext().getContentResolver();
    ...
    public class IcuDateTextViewReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            boolean updateFormatter = "android.intent.action.TIMEZONE_CHANGED".equals(action)
