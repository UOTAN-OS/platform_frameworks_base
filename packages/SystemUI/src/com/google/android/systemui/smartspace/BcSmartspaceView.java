...
public class BcSmartspaceView
        extends FrameLayout implements BcSmartspaceDataPlugin.SmartspaceTargetListener,
        ...
        public final void onPageScrollStateChanged(int state) {
            ...
            if (state == ViewPager2.SCROLL_STATE_IDLE) {
                if (mConfigProvider.isSwipeEventLoggingEnabled()
                        ...
                        && (cardAtPosition = mAdapter.getCardAtPosition(
                                    mSwipedCardPosition.intValue()))
                                != null) {
                    BcSmartspaceCardLogger.log(BcSmartspaceEvent.SMARTSPACE_CARD_SWIPE,
    ...
    public BcSmartspaceView(Context context, AttributeSet attrs) {
        ...
        mAodObserver = new ContentObserver(new Handler()) {
            ...
            public final void onChange(boolean selfChange) {
                mIsAodEnabled = Settings.Secure.getIntForUser(getContext().getContentResolver(),
                                        "doze_always_on", 0, getContext().getUserId())
                        == 1;
            }
        };
    ...
    public final void logSmartspaceEvent(
        ...
        if (event == BcSmartspaceEvent.SMARTSPACE_CARD_RECEIVED) {
            try {
                receivedLatencyMillis = (int) Instant.now()
                                                .minusMillis(target.getCreationTimeMillis())
                                                .toEpochMilli();
            } catch (ArithmeticException | DateTimeException e) {
                Log.e("BcSmartspaceView", "received_latency_millis will be -1 due to exception ",
                        e);
        ...
        BcSmartspaceCardLoggingInfo.Builder loggingInfoBuilder =
                new BcSmartspaceCardLoggingInfo.Builder()
                        ...
                        .setDisplaySurface(BcSmartSpaceUtil.getLoggingDisplaySurface(
    ...
    public final void onAttachedToWindow() {
        ...
        if (mViewPager != null) {
            if (mAdapter instanceof CardPagerAdapter) {
                ...
                if (TextUtils.equals(mAdapter.getUiSurface(),
                            BcSmartspaceDataPlugin.UI_SURFACE_LOCK_SCREEN_AOD)) {
                    try {
                        if (mBgHandler == null) {
                            throw new IllegalStateException("Must set background handler to avoid "
                                    + "making binder calls on main thread");
                        }
                        mBgHandler.post(() -> {
                            ContentResolver resolver = getContext().getContentResolver();
                            int userId = getContext().getUserId();
                            mIsAodEnabled = Settings.Secure.getIntForUser(
                                                    resolver, "doze_always_on", 0, userId)
                                    == 1;
                            resolver.registerContentObserver(
                                    Settings.Secure.getUriFor("doze_always_on"), false,
                                    mAodObserver, -1);
                        });
                    } catch (Exception e) {
                        Log.w("BcSmartspaceView",
                                "Unable to register Doze Always on content observer.", e);
                    }
                }
    ...
    public final void onFinishInflate() {
        ...
        if (pager instanceof ViewPager) {
            ...
            if (mViewPager2 != null) {
                ...
                if (cardRecyclerViewAdapter.smartspaceTargets.size() > 0) {
                    ...
                    mPreInflatedViewHolder = cardRecyclerViewAdapter.createViewHolder(
        ...
        if (mPagerDots != null) {
            int paddingStart = getResources().getDimensionPixelSize(
                    R.dimen.non_remoteviews_card_padding_start);
            mPagerDots.setPaddingRelative(paddingStart, mPagerDots.getPaddingTop(),
                    mPagerDots.getPaddingEnd(), mPagerDots.getPaddingBottom());
        }
    }
    ...
    public final void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int desiredHeight = getContext().getResources().getDimensionPixelSize(
    ...
    public final void onSmartspaceTargetsUpdated(List<? extends Parcelable> targets) {
        ...
        if (DEBUG) {
            Log.d("BcSmartspaceView",
                    "@" + Integer.toHexString(hashCode())
                            + ", onTargetsAvailable called. Callers = " + Debug.getCallers(5));
    ...
    public final void onVisibilityAggregated(boolean isVisible) {
        ...
        if (mDataProvider != null) {
            mDataProvider.getEventNotifier().notifySmartspaceEvent(
    ...
    public final void setDozeAmount(float dozeAmount) {
        ...
        if (mAdapter.getHasDifferentTargets() && mAdapter.getSmartspaceTargets() != previousTargets
                && mAdapter.getCount() > 0) {
            if (mAdapter instanceof CardRecyclerViewAdapter) {
                setSelectedPage(0);
            } else {
        ...
        int displaySurface = BcSmartSpaceUtil.getLoggingDisplaySurface(
                mAdapter.getUiSurface(), mAdapter.getDozeAmount());
        if (displaySurface == -1) {
            return;
        }
    ...
    public final void setDozing(boolean dozing) {
        if (!dozing && mSplitShadeEnabled && mAdapter.getHasAodLockscreenTransition()
    ...
    public final void setSelectedPage(int i) {
        if (mViewPager != null) {
            ...
            mViewPager2.post(() -> { mViewPager2.setCurrentItem(i, false); });
        }
    ...
    public final void setTargets(boolean z, int i, View view, int i2) {
        ...
        mLastReceivedTargets.addAll((Collection) mAdapter.getSmartspaceTargets()
                        .stream()
