...
public class WeatherSmartspaceView
    ...
    public WeatherSmartspaceView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            mIconSize = ta.getDimensionPixelSize(1,
                    context.getResources().getDimensionPixelSize(
                            R.dimen.enhanced_smartspace_icon_size));
            int iconInset = ta.getDimensionPixelSize(0,
                    context.getResources().getDimensionPixelSize(
    ...
    public void onSmartspaceTargetsUpdated(List<? extends Parcelable> targets) {
        ...
        BcSmartspaceCardLoggingInfo.Builder builder =
                new BcSmartspaceCardLoggingInfo.Builder()
                        ...
                        .setUid(-1)
                        .setDimensionalInfo(BcSmartspaceCardLoggerUtil.createDimensionalLoggingInfo(
                                templateData));
        ...
        if (hasValidTemplate) {
            ...
            CharSequence text =
                    ...
                    : "";
            CharSequence contentDescription = subItemInfo.getIcon() != null
            ...
            if (subItemInfo.getTapAction() != null && isClickable()) {
                BcSmartspaceDataPlugin.SmartspaceEventNotifier notifier =
                        mDataProvider != null ? event
                        -> mDataProvider.notifySmartspaceEvent(event)
                        : null;
                BcSmartSpaceUtil.setOnClickListener(mView, target, subItemInfo.getTapAction(),
                        notifier, "WeatherSmartspaceView", mLoggingInfo, 0);
            }
        } else {
            ...
            ContentDescriptionUtil.setFormattedContentDescription("WeatherSmartspaceView", mView,
                    headerAction.getTitle(), headerAction.getContentDescription());
            Drawable iconDrawable = BcSmartSpaceUtil.getIconDrawableWithCustomSize(
                    headerAction.getIcon(), getContext(), mIconSize);
            mIconDrawable.setIcon(iconDrawable);
            mView.setCompoundDrawablesRelative(mIconDrawable, null, null, null);
            BcSmartspaceDataPlugin.SmartspaceEventNotifier notifier = mDataProvider != null ? event
                    -> mDataProvider.notifySmartspaceEvent(event)
                    : null;
            BcSmartSpaceUtil.setOnClickListener(mView, target, headerAction, notifier,
                    "WeatherSmartspaceView", mLoggingInfo, 0);
        }
        if (mRemoveTextDescent) {
            int paddingBottom = mTextDescentExtraPadding
    ...
    private class WeatherSmartspaceViewObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            boolean isAodEnabled =
                    Settings.Secure.getIntForUser(outer.getContext().getContentResolver(),
                            "doze_always_on", 0, outer.getContext().getUserId())
                    == 1;
