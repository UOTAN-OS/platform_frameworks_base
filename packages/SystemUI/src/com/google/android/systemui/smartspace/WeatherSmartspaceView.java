...
public class WeatherSmartspaceView
        extends LinearLayout implements BcSmartspaceDataPlugin.SmartspaceTargetListener,
    ...
    public final void onAttachedToWindow() {
        ...
        if (TextUtils.equals(mUiSurface, BcSmartspaceDataPlugin.UI_SURFACE_LOCK_SCREEN_AOD)) {
            try {
                if (mBgHandler == null) {
                    throw new IllegalStateException("Must set background handler to avoid making "
                ...
                mBgHandler.post(() -> {
                    getContext().getContentResolver().registerContentObserver(
                            ...
                            mAodSettingsObserver, -1);
                });
            } catch (Exception e) {
                Log.w("WeatherSmartspaceView",
                        "Unable to register DOZE_ALWAYS_ON content observer: ", e);
            }
        ...
        if (hasValidTemplate || target.getHeaderAction() != null) {
            ...
            if (!hasValidTemplate) {
                ...
                if (headerAction == null) {
                    ...
                    mIconDrawable.setIcon(BcSmartSpaceUtil.getIconDrawableWithCustomSize(
                            headerAction.getIcon(), getContext(), mIconSize));
                    mView.setCompoundDrawablesRelative(mIconDrawable, null, null, null);
                    ContentDescriptionUtil.setFormattedContentDescription("WeatherSmartspaceView",
                            mView, headerAction.getTitle(), headerAction.getContentDescription());
                    if (!TextUtils.equals(mUiSurface, BcSmartspaceDataPlugin.UI_SURFACE_DREAM)) {
                        BcSmartSpaceUtil.setOnClickListener(mView, target, headerAction,
                                mDataProvider != null ? mDataProvider.getEventNotifier() : null,
                                "WeatherSmartspaceView", mLoggingInfo, 0);
                    }
                }
                ...
                if (subItemInfo == null) {
                    ...
                    if (subItemInfo.getTapAction() != null
                        ...
                        BcSmartSpaceUtil.setOnClickListener(mView, target,
            ...
            if (mRemoveTextDescent) {
                mView.setPaddingRelative(0, 0, 0,
    ...
    public final void setDozeAmount(float dozeAmount) {
        ...
        if (loggingSurface != 3 || mIsAodEnabled) {
            ...
            BcSmartspaceCardLogger.log(BcSmartspaceEvent.SMARTSPACE_CARD_SEEN,
                    new BcSmartspaceCardLoggingInfo(builder));
        }
    }
    ...
    public WeatherSmartspaceView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray obtainStyledAttributes = context.getTheme().obtainStyledAttributes(
                attrs, R.styleable.WeatherSmartspaceView, 0, 0);
        try {
            int iconSize = obtainStyledAttributes.getDimensionPixelSize(1,
                    context.getResources().getDimensionPixelSize(
                            R.dimen.enhanced_smartspace_icon_size));
            int iconInset = obtainStyledAttributes.getDimensionPixelSize(0,
                    context.getResources().getDimensionPixelSize(
                            R.dimen.enhanced_smartspace_icon_inset));
